...
class SST_Coupon_Manager {
    ...
    /**
     * Update coupon discount amount and type
     *
     * @param string $coupon_code The coupon code
     * @param float $new_amount New discount amount
     * @param string $discount_type Type: 'percent' or 'fixed_cart'
     * @return bool|WP_Error Success or error
     */
    public function update_coupon($coupon_code, $new_amount, $discount_type = 'percent') {
        $coupon = $this->get_coupon_by_code($coupon_code);
        ...
        error_log(sprintf('SST Affiliate: Updated coupon %s to %.2f %s', $coupon_code, $new_amount, $discount_type));
        return true;
    }

    /**
     * Disable a coupon (set expiry to yesterday)
     *
     * @param string $coupon_code The coupon code to disable
     * @return bool|WP_Error Success or error
     */
    public function disable_coupon($coupon_code) {
        ...
        // Set expiry to yesterday to disable it
        $yesterday = date('Y-m-d', strtotime('-1 day'));
        $coupon->set_date_expires($yesterday);
        $coupon->save();

        error_log(sprintf('SST Affiliate: Disabled coupon %s', $coupon_code));
        return true;
    }

    /**
     * Enable a coupon (remove expiry or set to future date)
     *
     * @param string $coupon_code The coupon code to enable
     * @param int $days_until_expiry Number of days until expiry (0 = never expires)
     * @return bool|WP_Error Success or error
     */
    public function enable_coupon($coupon_code, $days_until_expiry = 0) {
        $coupon = $this->get_coupon_by_code($coupon_code);
        ...
        if ($days_until_expiry > 0) {
            $expiry_date = date('Y-m-d', strtotime("+{$days_until_expiry} days"));
            $coupon->set_date_expires($expiry_date);
        } else {
        ...
        error_log(sprintf('SST Affiliate: Enabled coupon %s', $coupon_code));
        return true;
    }
}
